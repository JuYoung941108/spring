...
class AppContext {
    ...
    private void doResource() {
        // 자동으로 객체 연결
        // map에 저장된 객체의 IV 중 @Resource가 붙어있으면 map에서 IV 이름에 맞는 이름를 찾아서 연결함(객체 주소를 iV에 저장)
        try {
    ...
    private void doAutowired() {
        // 자동으로 객체 연결
        // map에 저장된 객체의 IV 중 @Autowired가 붙어있으면 map에서 IV 타입에 맞는 객체를 찾아서 연결함(객체 주소를 iV에 저장)
        try {
            for(Object bean:map.values()) {
                for(Field fld:bean.getClass().getDeclaredFields()) {
    ...
    private void doComponentScan() {
        try {
            ...
            // 3. @Component이 붙어있으면 객체 생성 -> map 저장
                if(component!=null){
